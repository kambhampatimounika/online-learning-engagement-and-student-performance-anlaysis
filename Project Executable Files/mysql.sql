...
SELECT engagement_level, COUNT(DISTINCT id_student) AS total_students, 
...
FROM online_education_dataset GROUP BY engagement_level ORDER BY dropout_rate DESC;
SELECT performance_level, COUNT(DISTINCT id_student) AS total_students,
 SUM(CASE WHEN dropout_flag = 1 THEN 1 ELSE 0 END) AS dropout_students,
 ROUND(100.0 *SUM(CASE WHEN dropout_flag = 1 THEN 1 ELSE 0 END) / COUNT(*), 2) AS dropout_rate 
 FROM online_education_dataset GROUP BY performance_level ORDER BY dropout_rate DESC;
 SELECT region, COUNT(DISTINCT id_student) AS total_students,
 ROUND(AVG(avg_score), 2) AS average_score, ROUND(AVG(total_clicks), 2) AS average_clicks 
 FROM online_education_dataset GROUP BY region ORDER BY total_students DESC;
SELECT highest_education, COUNT(DISTINCT id_student) AS total_students, ROUND(AVG(avg_score), 2) AS average_score, ROUND(AVG(total_clicks), 2) AS average_clicks 
FROM online_education_dataset GROUP BY highest_education ORDER BY average_score DESC;
